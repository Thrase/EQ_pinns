...
def wexact_t_auto(z):
    grad, = torch.autograd.grad(wexact(z).sum(), z, create_graph=True)
...
w = Feedforward(2,hidden_dim).to(device)  # displacement network
#v = Feedforward(2,hidden_dim).to(device)  # velocity network
...
def my_loss(z, z_x0, z_x1, z_t01, z_t02):
    #_, t_ψ0 = z_ψ0.split(1, dim=1)
    loss = wave_relation(z).sum() / z.shape[0]
    #loss += fault_bc(z_x0).abs().square().sum() / z_x0.shape[0]
    loss += (μ*u(z_x0) - fault_str(z_x0) - fault_src(z_x0)).abs().square().sum() / z_x0.shape[0]
    #loss += (w(z_x0) - wexact(z_x0)).abs().square().sum() / z_x0.shape[0]
    loss += (Z*v(z_x1) + μ*u(z_x1)).abs().square().sum() / z_x1.shape[0]
    
    #loss += (w(z_x1) - wexact(z_x1)).abs().square().sum() / z_x1.shape[0]
    #loss += (u(z_x1) - u_e(z_x1)).abs().square().sum() / z_x1.shape[0]
    loss += (w(z_t01) - wexact(z_t01)).abs().square().sum() / z_t01.shape[0]
    #loss += (u(z_t01) - wexact_x(z_t01)).abs().square().sum() / z_t01.shape[0]
    loss += (v(z_t02) - wexact_t(z_t02)).abs().square().sum() / z_t02.shape[0]
    #loss += state_relation(z_ψ).abs().square().sum() / z_ψ.shape[0]
    #loss += (ψ(t_ψ0) + ψ_e(z_ψ0)).abs().square().sum() / z_ψ0.shape[0]
    
    #loss += state_evoln(z_ψ).abs().square().sum() / z_ψ.shape[0]
    return loss
...
anim = @animate for i=1:length(tfine)
  ...
  p1 = plot(xfine, wNet[i], size=(1000, 750),color=color[1],
            lw=1.5, legend=:topright,#ylims=(-0.0, 1.0),
  ...
  p2 = plot(xfine, uNet[i], size=(1000, 750),color=color[1],
      ...
      plot!(twinx(),xfine, uerr[i], #ylims=(-0.001, 0.001),
  ...
  p3 = plot(xfine, vNet[i], size=(1000, 750),color=color[1],
      ...
      plot!(twinx(),xfine, verr[i], #ylims=(0.004, 0.008),
...
gif(anim, "rate_and_state(1D_lin)fps30.gif", fps = 30)
""")
